...
vec4 calcDirLight(vec3 normal, vec3 viewDir){
    ...
    return ambient + (diffuse + specular) * calcShadow(vPosition * uDirLight.shadowMVPMatrix, uDirLight.shadowMap, uDirLight.shadowEnabled);
}
...
vec4 calcPointLight(PointLight pointLight, vec3 normal, vec3 viewDir){
    ...
    return ambient + (diffuse + specular) * calcShadow(vPosition * pointLight.shadowMVPMatrix, pointLight.shadowMap, pointLight.shadowEnabled);
}
